...
## Purpose
Lucent 配置（providers / logMode / 保留期 / bodyRewrites 等）的持久化与导入导出契约：配置以单行 JSON blob 存于应用 SQLite 库（`lucent.db`）的 `config` 表，与日志同库（单一备份目标）；DB 路径固定为 `LUCENT_DB_PATH || CONFIG_DIR/lucent.db`（不依赖 config 内容）；首次启动从 legacy `config.json` 一次性迁移；提供 `GET /api/config/export`、`POST /api/config/import` 用于 SQL 备份与迁移。
## Requirements
